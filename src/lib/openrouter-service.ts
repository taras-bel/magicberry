...
export class OpenRouterService {
    ...
    try {
      const response = await fetch(this.baseUrl, {
        ...
        headers: {
          ...
          'X-Title': 'Magic Berry Recipe Assistant',
        },
  ...
  async generateRecipe(ingredients: string[], options: {
    ...
    const systemPrompt = `Ты профессиональный повар-консультант компании Magic Berry, специализирующейся на натуральных вяленых ягодах, фруктах и овощах. 

Твоя задача - создавать практичные, подробные и вкусные рецепты, которые подчеркивают качество натуральных продуктов Magic Berry.
...
3. Ингредиенты - точный список с КОЛИЧЕСТВОМ (в граммах, штуках, столовых ложках и т.д.)
   - Если в списке есть продукты Magic Berry (вяленые ягоды, фрукты, овощи, сиропы), указывай их полное название: "Вяленая клюква от Magic Berry", "Натуральный ягодный сироп от Magic Berry"
...
ВАЖНО: Если в рецепте используются продукты Magic Berry (вяленые ягоды, фрукты, овощи или сиропы), обязательно указывай их с упоминанием бренда "от Magic Berry" в списке ингредиентов.
...
- [если продукт Magic Berry, то: Вяленая клюква от Magic Berry - 50г]
- [и так далее]
...
Сложность: [${difficultyText}]`;
  ...
  async processChatMessage(
    ...
    const systemPrompt = `Ты виртуальный помощник Magic Berry - магазина натуральных вяленых ягод, фруктов и овощей. 
